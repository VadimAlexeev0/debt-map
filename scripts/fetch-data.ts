...
// ISO codes for countries / territories that report to IMF but may not appear
// in the 110m Natural Earth map (small islands, city-states, etc.)
const SMALL_COUNTRY_CODES = new Set([
  "AND", "ABW", "ATG", "BHR", "BRB", "CPV", "COM", "DMA", "FSM", "GRD",
  "HKG", "KIR", "LCA", "LIE", "MAC", "MDV", "MLT", "MHL", "MUS", "NRU",
  "PLW", "KNA", "SMR", "STP", "SYC", "SGP", "SLB", "TON", "TUV", "VCT",
  "WSM",
]);

// IMF aggregate / regional codes to filter out
const IMF_AGGREGATE_CODES = new Set([
  "WEO", "WEOWORLD", "OEMDC", "ADVEC", "APD", "APDE", "APDA", "APDC", "APDS",
  "APDBX", "APDWX", "LAC", "LACX", "MCD", "MCDX", "MECA", "MECAX", "MENA",
  "MENAP", "MENAPX", "MENAx", "SSA", "SSAX", "CIS", "CISX", "EUR", "EU",
  "EURO", "EEA", "AFR", "ASI", "AMC", "AME", "OCE", "WLD", "WLDX",
  "MAE", "NMQ", "WHQ", "EAQ", "APQ", "WEQ", "DA", "SAQ", "SMQ", "EUQ",
  "NAQ", "AS5", "WE", "SEQ", "AFQ", "SSQ", "CBQ", "PIQ", "CMQ", "OAE",
  "AZQ", "MEQ", "EEQ", "EDE", "CAQ", "WBG", "EME", "EME2", "EMDE", "EMDEV",
  "EUQ", "OIE", "MIDE", "SME", "AFRX", "MCDX", "APDX", "LACX", "SSAX",
  "EMX", "EUE", "EUX", "A5M", "ASEAN", "ASEAN5", "SAARC", "SADC", "ECOWAS",
  "GCC", "CEMAC", "WAEMU", "CAEMC", "CEMACX", "WAEMUX", "CAEMCX", "EAC",
  "SACU", "CEMAC", "UEMOA", "OPEC", "OPECN", "OPECS", "G7", "G20", "G20X",
  "G7X", "G7E", "G7X", "G20E", "G20X", "ADV", "EMG", "LIDC", "LIDCX",
  "FCS", "FCSX", "HIPC", "HIPCX", "PRGF", "PRGFX", "SBA", "SBAX", "EFF",
  "EFFX", "ESF", "ESFX", "FCL", "FCLX", "PLL", "PLLX", "RAP", "RAPX",
  "RTP", "RTPX", "CCRT", "CCRTX", "SCF", "SCFX", "PCI", "PCIX", "PSI",
  "PSIX", "SIP", "SIPX", "FSSI", "FSSIX", "MUIC", "MUICX", "SML", "SMLX",
  "OAL", "OALX", "PAL", "PALX", "SAL", "SALX", "SPL", "SPLX", "TAL",
  "TALX", "MUI", "MUIX", "MUI2", "MUI2X", "OAL", "OALX", "PAL", "PALX",
  "SAL", "SALX", "SPL", "SPLX", "TAL", "TALX",
]);
...
const countryNames: Record<string, string> = {
  ...
  CHN: "China", COL: "Colombia", COM: "Comoros", COD: "Congo, Dem. Rep.",
  COG: "Congo, Rep.", CRI: "Costa Rica", CIV: "Cote d'Ivoire", HRV: "Croatia",
  ...
  GUY: "Guyana", HTI: "Haiti", HND: "Honduras", HKG: "Hong Kong SAR",
  ...
  KOR: "Korea, Republic of", UVK: "Kosovo", KWT: "Kuwait", KGZ: "Kyrgyz Republic",
  LAO: "Laos", LVA: "Latvia", LBN: "Lebanon", LSO: "Lesotho",
  LBR: "Liberia", LBY: "Libya", LTU: "Lithuania", LUX: "Luxembourg",
  MAC: "Macao SAR", MDG: "Madagascar", MWI: "Malawi", MYS: "Malaysia",
  MDV: "Maldives", MLI: "Mali", MLT: "Malta", MHL: "Marshall Islands",
  MRT: "Mauritania", MUS: "Mauritius", MEX: "Mexico", FSM: "Micronesia, Fed. States of",
  ...
  ROU: "Romania", RUS: "Russian Federation", RWA: "Rwanda", WSM: "Samoa",
  ...
  SVK: "Slovak Republic", SVN: "Slovenia", SLB: "Solomon Islands", SOM: "Somalia",
  ...
  SYR: "Syria", TWN: "Taiwan Province of China", TJK: "Tajikistan", TZA: "Tanzania",
  THA: "Thailand", TLS: "Timor-Leste", TGO: "Togo", TON: "Tonga",
  TTO: "Trinidad and Tobago", TUN: "Tunisia", TUR: "Turkiye", TKM: "Turkmenistan",
...
// Codes that Natural Earth uses but IMF uses a different code for
const CODE_MAPPING: Record<string, string> = {
  // Kosovo: IMF uses UVK, Natural Earth uses KOS
  UVK: "KOS",
};
...
function isValidCountryCode(code: string): boolean {
  if (IMF_AGGREGATE_CODES.has(code)) return false;
  if (code.length !== 3) return false;
  // Heuristic: aggregate codes often contain numbers or are all uppercase non-standard
  // Real ISO codes are 3 letters. We'll additionally require the code to either be
  // in our known country list or in the small-country list.
  if (countryNames[code] || SMALL_COUNTRY_CODES.has(code)) return true;
  // Also allow if it maps to a known code
  if (CODE_MAPPING[code] && (countryNames[CODE_MAPPING[code]] || SMALL_COUNTRY_CODES.has(CODE_MAPPING[code]))) return true;
  // Filter out obvious aggregates (3-letter codes not in our lists)
  return false;
}
...
async function main() {
  ...
  for (const [code, years] of Object.entries(debtSeries)) {
    ...
    // Use mapped code for display if needed
    const displayCode = CODE_MAPPING[code] || code;
    const name = countryNames[code] || countryNames[displayCode] || code;
